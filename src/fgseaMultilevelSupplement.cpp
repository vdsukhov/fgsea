...
void EsRuler::updateSample() {
    ...
    enrichmentScores.push_back(stats[0].first);
    if (find(posEsIndxs.begin(), posEsIndxs.end(), stats[0].second) != posEsIndxs.end()) {
        totalPosEsCount--;
    }
...
pair<double, bool> EsRuler::getPvalue(double ES, double eps, bool sign) {
    ...
    double adjLog = betaMeanLog(sampleSize, sampleSize);
    double adjLogPval = indx * adjLog;
